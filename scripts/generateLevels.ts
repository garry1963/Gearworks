...
export function generateLevelFromSpec(spec: SpecItem): LevelData | null {
  ...
  for (let attempt = 0; attempt < 50; attempt++) {
    ...
    if (targetCount === 1) {
      // Snaking chain across horizontal passes
...
export function runBatchGeneration() {
  console.log(`Starting generation of 100 levels into ${LEVELS_DIR}...`);
  let successCount = 0;
  const failures: number[] = [];

  for (const spec of levelDatabase.levels as SpecItem[]) {
    const filename = `level_${spec.id.toString().padStart(3, '0')}.json`;
    const filePath = path.join(LEVELS_DIR, filename);

    if (spec.id <= 10 && fs.existsSync(filePath)) {
      try {
        const existingData = JSON.parse(fs.readFileSync(filePath, 'utf8'));
        const report = validateLevel(existingData);
        if (report.isValid && report.solutionSolvesLevel) {
          successCount++;
          console.log(`Preserved handcrafted Level ${spec.id} (${spec.name})`);
          continue;
        }
      } catch (e) {
        console.warn(`Error reading existing Level ${spec.id}, will regenerate.`);
      }
    }

    const generated = generateLevelFromSpec(spec);
    if (generated) {
      fs.writeFileSync(filePath, JSON.stringify(generated, null, 2), 'utf8');
      successCount++;
      console.log(`Generated & Validated Level ${spec.id} (${spec.name}) [${spec.difficulty}]`);
    } else {
      failures.push(spec.id);
      console.error(`FAILED to generate Level ${spec.id} (${spec.name})`);
    }
  }

  console.log(`\nGeneration Complete: ${successCount}/100 successful.`);
  if (failures.length > 0) {
    console.error(`Failed level IDs: ${failures.join(', ')}`);
    process.exit(1);
  }
}
